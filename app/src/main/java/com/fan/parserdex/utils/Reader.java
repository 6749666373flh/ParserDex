...
import static com.fan.parserdex.utils.TransformUtils.reverseBytes;
...
public class Reader {
    ...
    public byte[] readBigEndian(int count) throws IOException {
        byte[] b = new byte[count];
        int read = in.read(b);
        if (read == -1) throw new EOFException();
        if (!isLittleEndian) return b;
    ...
    public String readHexString(int count) throws IOException {
        return TransformUtils.byte2HexStr(readBigEndian(count));
    }
